...
png("Plots/nrc/Irby/total.png", width = 1200,height = 700)
barplot(
  sort(colSums(prop.table(nrcInput[, 1:8]))), 
  horiz = TRUE, 
  cex.names = 0.7, 
  las = 1, 
  main = "Emotions in Irby", xlab="Percentage"
)
dev.off()

png("Plots/nrc/Irby/Sent.png", width = 1200,height = 700)
barplot(
  sort(colSums(prop.table(nrcInput[, 9:10]))), 
  horiz = TRUE, 
  cex.names = 0.7, 
  las = 1, 
  main = "Sentiment in Irby", xlab="Percentage"
)
dev.off()
